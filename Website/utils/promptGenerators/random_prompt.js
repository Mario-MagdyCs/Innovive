...
// Load JSON
const dataPath = path.join(__dirname, 'complete_materials_dataset.json');
...
//     if (!entry) {
//       console.warn(`⚠️ Material not found in dataset: "${material}"`);
//       continue; // skip to next
//     }

//     // Select a random project with clear description
//     const randomProject = entry.projects[Math.floor(Math.random() * entry.projects.length)];
//     const projectName = randomProject.name;
//     const projectDescription = randomProject.description;
//     const projectUsage = randomProject.usage;

//     // Short, clear, high-quality prompt
//     const prompt = `Creative DIY ${projectName} using ${material}. Purpose: ${projectUsage}. ${projectDescription}. High-quality, eco-friendly, visually appealing.`;
...
// // ✅ Example usage
// // const selectedMaterials = ["glass_jar", "plastic_straws"];
// // console.log(generateRandomPrompt(selectedMaterials));
...
//   ][Math.floor(Math.random() * 5)];
  ...
  return `Generate a diy ${selectedCraft} made from ${materials.join(" and ")} . High quality, Realistic,9k .`;
}
